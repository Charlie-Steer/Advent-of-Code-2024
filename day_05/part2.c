...
void print_update(int update[UPD_COLS]);
...
void order_update(int update[UPD_COLS], int updates_list[UPD_ROWS][UPD_COLS], int order_list[ORD_ROWS][ORD_COLS]) {
	bool update_valid = true;
	...
	for (int j = 0; (main_number = update[j]) != 0; j++) {
		for (int k = 0; (other_number = update[k]) != 0; k++) {
			bool does_pair_respect_ordering = check_if_pair_respects_ordering(main_number, j, other_number, k, order_list);
			if (!does_pair_respect_ordering) {
				update[j] = other_number;
				update[k] = main_number;
				break;
				printf("NO\n");
			}
		}
...
void order_update_according_to_rules(int update[UPD_COLS], int updates[UPD_ROWS][UPD_COLS], int orders[ORD_ROWS][ORD_COLS]) {
	print_update(update);
	while (!check_if_update_valid(update, updates, orders)) {
		order_update(update, updates, orders);
		print_update(update);
	}
}

int get_sum_of_middle_numbers_of_invalid_updates(int updates[UPD_ROWS][UPD_COLS], int orders[ORD_ROWS][ORD_COLS]) {
	int sum_of_middle_numbers_of_invalid_updates = 0;
	for (int i = 0; i < UPD_ROWS; i++) {
		int *update = updates[i];
		bool is_update_valid = check_if_update_valid(update, updates, orders);
		if (!is_update_valid) {
			order_update_according_to_rules(update, updates, orders);
			sum_of_middle_numbers_of_invalid_updates += get_middle_number(update);
		}
		printf("\n");
	}
	return (sum_of_middle_numbers_of_invalid_updates);
}
...
int main(void) {
	...
	int sum_b = get_sum_of_middle_numbers_of_invalid_updates(updates, orders);
	printf("answer b:\n%d\n", sum_b);
